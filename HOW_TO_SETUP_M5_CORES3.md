...
`m5_core_s3_scripts/arduino_ino/credentials.example.h`をコピーして `m5_core_s3_scripts/arduino_ino/credentials.h`を作成

`credentials.h` を編集して実際の値を入れてください

```cpp
const char* ssid1= "<HOME_SSID>";
const char* pass1= "<HOME_PASSWORD>";
const char* ssid2= "<TRAVEL_SSID>";
const char* pass2= "<TRAVEL_PASSWORD>";
```

`m5_core_s3_scripts/arduino_ino/config.h`を編集して実際の値を入れてください

```cpp
#define USER_NAME   "あなたの名前をアルファベットで入れてください"

// ぷちの名前
#define CHARACTOR_ID "ぷちの名前をアルファベットで入れてください"

// 家だけで使う場合は同じものを入れてください
// 家でのぷちのIPアドレス (e.g.) 192.168.1.XXのラストIP
#define HOME_IP_BEGIN "192.168.1"
#define HOME_IP_LAST 12
// 外出用ルータのぷちのIPアドレス (e.g.) 192.168.8.XXのラストIP
